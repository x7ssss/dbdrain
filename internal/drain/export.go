...
type Exporter struct {
	...
	colocationTenants map[int]map[string]bool // colocation_id -> set of tenant distribution keys
...
func New(source any, schema *introspect.Schema, g *graph.Graph, sccs []graph.SCC, cfg Config) *Exporter {
	...
	return &Exporter{
		...
		colocationTenants: make(map[int]map[string]bool),
		resolver:          resolver,
...
func (e *Exporter) ExportAnchors(ctx context.Context, w io.Writer, anchors []Anchor) error {
	fmt.Fprintf(w, "-- Generated by dbdrain v1.0.0 on %s\n", time.Now().UTC().Format(time.RFC3339))
	...
	if e.engine == db.EngineMySQL {
		...
		// When generating DDL, emit Citus distributed / reference table declarations
		if e.schema != nil && e.schema.HasCitus && len(e.schema.CitusTables) > 0 {
			for tbl := range e.schema.CitusTables {
				if !e.schema.IsPartitionedRoot(tbl) {
					citusDDL := introspect.GenerateCitusTableDDL(e.schema, tbl)
					if citusDDL != "" {
						fmt.Fprintln(w, citusDDL)
						fmt.Fprintln(w, "")
					}
				}
			}
...
func (e *Exporter) ExportAnchorsToTarget(
	...
	// Ensure Citus distributed and reference tables are configured on target before streaming
	if e.schema != nil && e.schema.HasCitus && len(e.schema.CitusTables) > 0 {
		for tbl := range e.schema.CitusTables {
			citusDDL := introspect.GenerateCitusTableDDL(e.schema, tbl)
			if citusDDL != "" {
				_, _ = targetTx.Exec(ctx, citusDDL)
			}
		}
	}
...
func (e *Exporter) bfsParentsToTarget(
	...
	for len(queue) > 0 {
		...
		for _, edge := range e.g.OutEdges[table] {
			...
			_, toCol, ok := e.resolveParentEdge(edge)
			if !ok {
				continue
			}
			...
			colInfo := findColumnByName(parentCols, toCol)
			condition := ""
			if assoc != nil {
				condition = assoc.Condition()
			}
			condition = combineConditions(condition, e.getColocationCondition(parent))

			rows, err := queryByIDsWithCondition(ctx, sourceTx, e.engine, e.cfg.Schema, parent, colNames, toCol, colInfo, fkVals, 0, condition)
			if err != nil {
...
func (e *Exporter) bfsChildrenToTarget(
	...
	for len(queue) > 0 {
		...
		for _, edge := range e.g.InEdges[item.table] {
			...
			fromCol, _, ok := e.resolveChildEdge(edge)
			if !ok {
				continue
			}
			...
			colInfo := findColumnByName(childCols, fromCol)

			sampling := e.samplingConfig(child, fromCol)
			condition := e.getColocationCondition(child)
			rows, err := queryByIDsWithSamplingAndCondition(ctx, sourceTx, e.engine, e.cfg.Schema, child, colNames, fromCol, colInfo, fkVals, e.cfg.MaxRowsPerTable, sampling, condition)
			if err != nil {
...
func (e *Exporter) traverseUpstream(ctx context.Context, tx db.SourceTx, w io.Writer, rootTables []string) error {
	...
	walk = func(current string, visitedPath []string) error {
		for _, edge := range e.g.OutEdges[current] {
			...
			rows, err := queryByIDsWithCondition(ctx, tx, e.engine, e.cfg.Schema, parent, colNames, toCol, colInfo, fkVals, 0, condition)
			if err != nil {
...
func (e *Exporter) bfsChildren(ctx context.Context, tx db.SourceTx, w io.Writer, rootTables []string, _ int) error {
	...
	for len(queue) > 0 {
		...
		for _, edge := range e.g.InEdges[item.table] {
			...
			sampling := e.samplingConfig(child, fromCol)
			condition := e.getColocationCondition(child)
			rows, err := queryByIDsWithSamplingAndCondition(ctx, tx, e.engine, e.cfg.Schema, child, colNames, fromCol, colInfo, fkVals, e.cfg.MaxRowsPerTable, sampling, condition)
			if err != nil {
				return fmt.Errorf("query children (%s): %w", child, err)
			}
...
func (e *Exporter) recordTenantBoundary(table string, cols []introspect.Column, vals []any) {
	if e.schema == nil || !e.schema.HasCitus || e.schema.CitusTables == nil {
		return
	}
	ct, ok := e.schema.CitusTables[table]
	if !ok || ct.ColocationID == 0 || ct.DistributionColumn == "" {
		return
	}
	for i, c := range cols {
		if strings.EqualFold(c.Name, ct.DistributionColumn) && i < len(vals) && vals[i] != nil {
			vStr := fmt.Sprintf("%v", vals[i])
			if vStr != "" && vStr != "<nil>" {
				if e.colocationTenants == nil {
					e.colocationTenants = make(map[int]map[string]bool)
				}
				if e.colocationTenants[ct.ColocationID] == nil {
					e.colocationTenants[ct.ColocationID] = make(map[string]bool)
				}
				e.colocationTenants[ct.ColocationID][vStr] = true
			}
			break
		}
	}
...
func (e *Exporter) getColocationCondition(table string) string {
	if e.schema == nil || !e.schema.HasCitus || e.schema.CitusTables == nil {
		return ""
	}
	ct, ok := e.schema.CitusTables[table]
	...
	tenantsMap := e.colocationTenants[ct.ColocationID]
	if len(tenantsMap) == 0 {
		return ""
	}
	tenants := make([]string, 0, len(tenantsMap))
	...
	return fmt.Sprintf("%s IN (%s)", quoteIdent(ct.DistributionColumn), strings.Join(quoted, ", "))
}

func combineConditions(c1, c2 string) string {
	if c1 == "" {
		return c2
	}
	if c2 == "" {
	...
	return fmt.Sprintf("(%s) AND (%s)", c1, c2)
}

func (e *Exporter) resolveParentEdge(edge graph.Edge) (fromCol string, toCol string, ok bool) {
	if len(edge.ToColumns) == 1 && len(edge.FromColumns) == 1 {
		return edge.FromColumns[0], edge.ToColumns[0], true
	}
	if len(edge.ToColumns) == 2 && len(edge.FromColumns) == 2 && e.schema != nil && e.schema.HasCitus {
		fromCt := e.schema.CitusTables[edge.FromTable]
		toCt := e.schema.CitusTables[edge.ToTable]
		if fromCt != nil && toCt != nil && fromCt.TableType == introspect.CitusTableDistributed && toCt.TableType == introspect.CitusTableDistributed {
			var fCol, tCol string
			for _, c := range edge.FromColumns {
				if !strings.EqualFold(c, fromCt.DistributionColumn) {
					fCol = c
					break
				}
			}
			for _, c := range edge.ToColumns {
				if !strings.EqualFold(c, toCt.DistributionColumn) {
					tCol = c
					break
				}
			}
			if fCol != "" && tCol != "" {
				return fCol, tCol, true
			}
		}
	}
	return "", "", false
...
func (e *Exporter) resolveChildEdge(edge graph.Edge) (fromCol string, toCol string, ok bool) {
	if len(edge.FromColumns) == 1 && len(edge.ToColumns) == 1 {
		return edge.FromColumns[0], edge.ToColumns[0], true
	}
	if len(edge.FromColumns) == 2 && len(edge.ToColumns) == 2 && e.schema != nil && e.schema.HasCitus {
		fromCt := e.schema.CitusTables[edge.FromTable]
		toCt := e.schema.CitusTables[edge.ToTable]
		if fromCt != nil && toCt != nil && fromCt.TableType == introspect.CitusTableDistributed && toCt.TableType == introspect.CitusTableDistributed {
			var fCol, tCol string
...
func (e *Exporter) recordFKValues(table string, cols []introspect.Column, vals []any) {
	e.recordTenantBoundary(table, cols, vals)
...
func (e *Exporter) getFKValuesForEdge(table string, edge graph.Edge) []string {
	fromCol, _, ok := e.resolveParentEdge(edge)
	if ok && fromCol != "" {
		if e.fkData != nil && e.fkData[table] != nil && len(e.fkData[table][fromCol]) > 0 {
			out := make([]string, 0, len(e.fkData[table][fromCol]))
			for v := range e.fkData[table][fromCol] {
				out = append(out, v)
			}
			return out
		}
